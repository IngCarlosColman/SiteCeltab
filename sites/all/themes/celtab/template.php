...
function _celtab_add_css() {
  ...
  drupal_add_css($theme_path . '/css/colors/green.css');
  drupal_add_css($theme_path . '/css/celtab.css');

  $default_color = variable_get('celtab_theme_color');
  if (!empty($default_color)) {
    drupal_add_css($default_color);
  }
}

function celtab_preprocess_html(&$variables) {
  ...
  drupal_add_html_head(
      ...
      '#attributes' => array(
          ...
          ), 'celtab:viewport_meta'
  );
}

function celtab_preprocess_page(&$variables) {
  ...
  if ($page['sidebar_first']) {
    $content_class = "twelve alt2 columns";
    // $container_class = 'floated';
  }if ($page['sidebar_second']) {
    $content_class = 'twelve alt columns';
...
function celtab_format_comma_field($field_category, $node, $limit = NULL) {
...
function celtab_breadcrumb($variables) {
...
function celtab_table($variables) {
...
function celtab_status_messages(&$variables) {
...
function celtab_pager($variables) {
  $tags = $variables['tags'];
...
function celtab_tagclouds_weighted(array $vars) {
  $terms = $vars['terms'];
...
function celtab_preprocess_node(&$variables) {
  ...
  if (variable_get('node_submitted_' . $node->type, TRUE)) {
    ...
    $submitted = '<ul><li>' . t('by') . ' ' . $variables['name'] . ' - ' . format_date($node->created) . '</li>';

    if (!empty($variables['content']['field_tags'])) {
      $submitted .= '<li>' . celtab_format_comma_field('field_tags', $node) . '</li>';
    }
    if (!empty($node->comment_count)) {
      $submitted .= '<li><a href="' . url('node/' . $node->nid) . '">' . $node->comment_count . ' ' . t('Comments') . '</a></li>';
    }
    $submitted .= '</ul>';
...
function celtab_preprocess_views_view_table(&$vars) {
  $vars['classes_array'][] = 'standard-table';
}