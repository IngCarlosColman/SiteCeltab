...
function celtab_form_system_theme_settings_alter(&$form, $form_state) {

  $path = drupal_get_path('theme', 'celtab');
  ...
  $form['settings']['skin']['celtab_theme_color'] = array(
      '#title' => t('Theme color'),
      '#type' => 'textfield',
      '#default_value' => theme_get_setting('celtab_theme_color'),
      '#attributes' => array('class' => array('input color')),
      '#description' => t('Default color hex code is: #73B819'),
  );
  // bg background
  $dir = drupal_get_path('theme', 'celtab') . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . 'bg';
  ...
  $form['settings']['skin']['theme_background_image'] = array(
      ...
      '#type' => 'select',
      '#default_value' => theme_get_setting('theme_background_image', 'celtab'),
      '#options' => $bg_files,
      '#description' => t('All images background in <strong>!url</strong>', array('!url' => $dir)),
  );

  $form['#submit'][] = '_celtab_form_submit';
}

function _celtab_form_submit($form, &$form_state) {
  $values = $form_state['values'];

  if (!empty($values['celtab_theme_color'])) {
    _save_css_color_file($values['celtab_theme_color']);
  }
}
...
function _save_css_color_file($color) {
  ...
  variable_set('celtab_theme_color', $filepath);
}
